...
class Suggestions {
  ...
  static List<dynamic>? suggestRow(List<List<dynamic>> rows, List<String> headers) {
    if (rows.isEmpty) return null;
    final last = rows.last;
    final out = List<dynamic>.filled(headers.length, '');
    final idxProg = headers.indexWhere((h) => h.toLowerCase().contains('progres'));
    ...
    final idxFecha = headers.indexWhere((h) => h.toLowerCase().contains('fecha'));
