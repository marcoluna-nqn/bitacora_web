...
// Estado inmutable con helpers pedidos por el editor: withHeaders, withAppendedRows, withCell, withNewEmptyRow.
...
class TableState {
  ...
  TableState._internal(this.headers, this.rows, this.savedAt);
  ...
  // ------------------- Helpers funcionales (solicitados) -------------------
  ...
  TableState withAppendedRows(List<List<String>> chunk) {
    ...
    return TableState(
        headers: headers, rows: merged, savedAt: DateTime.now().toUtc());
  }
  ...
  TableState withCell(int row, int col, String value) {
    ...
    return TableState(
        headers: headers, rows: newRows, savedAt: DateTime.now().toUtc());
  }
  ...
  TableState trim({bool headersToo = true, bool cells = true}) {
    ...
    final newRows = cells
        ...
        : rows;
    return TableState(
        headers: newHeaders, rows: newRows, savedAt: DateTime.now().toUtc());
  }
