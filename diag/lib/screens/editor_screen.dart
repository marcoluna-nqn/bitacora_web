...
// - Filas alternas, foco visible, edición al tocar/doble click
// - Adjuntos por fila (file_selector) y exportación XLSX real con fotos
// - Barra de adjuntos entre encabezados y filas, por fila enfocada
// - GPS y Dictado a celda (geolocator / speech_to_text)
// - Backup/Import JSON, Undo/Redo, Agregar/Borrar fila/columna
// - Enviar por correo (flutter_email_sender), fallback mailto:/share_plus
...
class _EditorScreenState extends State<EditorScreen> {
  ...
  static const int _initialCols = 10;
  static const int _initialRows = 10;
  ...
  // -------- adjuntos (conteo general para badge) --------
  ...
  Future<void> _exportXlsx() async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (!mounted) return;
        messenger?.showSnackBar(SnackBar(content: Text('Descargado: $_lastSavedName')));
      } else {
    ...
    try {
      ...
      if (!kIsWeb && path != null) {
        try {
          ...
          if (!mounted) return;
          messenger?.showSnackBar(const SnackBar(content: Text('Correo enviado')));
          return;
        } catch (_) {
          // fallback
        }
        ...
        if (ok) {
          ...
          messenger?.showSnackBar(const SnackBar(content: Text('No se pudo abrir el cliente de correo')));
        }
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<Uint8List> _buildXlsxBytesWithPhotos() async {
    ...
    try {
      ...
      // estética mínima + bordes + autofit
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading
        ...
        children: [
          _buildHeader(cs),
          _buildAttachmentsBar(cs), // barra entre encabezados y filas
  ...
  Widget _buildAttachmentsBar(ColorScheme cs) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Columna índice visual y acción rápida
          SizedBox(
            width: _indexColW,
            child: Column(
              ...
              children: [
                ...
                InkWell(
                  ...
                  child: const Padding(
                    ...
                    child: Icon(Icons.add_photo_alternate, size: 18),
                  ),
                ),
              ],
            ),
  ...
  Widget _cell(int r, int c, ColorScheme cs) {
    ...
    final content = (_isEditing && focused)
      ...
      child: TextField(
        ...
        decoration:
        const InputDecoration(isDense: true, border: InputBorder.none, contentPadding: EdgeInsets.zero),
  ...
  Future<void> _pickAttachmentsForFocusedRow() async {
    ...
    final r = _focus.$1;
    try {
      final typeImages = XTypeGroup(label: 'Imágenes', extensions: const ['png', 'jpg', 'jpeg', 'webp', 'heic']);
      final files = await openFiles(acceptedTypeGroups: [typeImages]);
